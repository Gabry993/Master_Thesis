...
The original relloc system was applied to control a flying robot \cite{gromov2018robot}. In that context, the feedback user had while controlling the drone with pointing consisted of the drone itself. However, that approach is not suitable for a slow moving ground robot, as the kobuki. Here the turret system comes in play.\\
With our demo we demonstrate that the laser turret approach is suitable to face the mobile robot navigation task. Figure \ref{fig:kgtgdSetup} the setup of the demo: we have the user wearing the IMU device and the kobuki equipped with our turret. The steps composing the demo are the following:
\begin{itemize}
    \item the user does the relloc (figure \ref{fig:kgtgdRelloc});
    \item an audio feedback alerts the user that the relloc is done and he can now move the laser with pointing;
    \item the user points the place he wants the kobuki to reach for three second (figure \ref{fig:kgtgdPointing});
    \item the kobuki moves to that point, while the laser keeps pointing at it. (figure \ref{fig:kgtgdGoal})
\end{itemize}
Figure \ref{fig:kgtgdGoal} gives also a qualitative feedback: the user is able to drive the kobuki through the cones easily and precisely



\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/kgtgdSetup.png}%
	\caption{Kobuki Go to Goal Demo: Setup}
	\label{fig:pointingExpSetup}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/kgtgdPointing.png}%
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/kgtgdGoal.png}%
