...
The relative localization procedure goal is to estimate the pose of the turret's frame in the reference frame of the human. The approach we use is the one proposed in \cite{gromov2018robot}.\\
We want to find a four-dimensional vector:
\begin{align}
	\rho = [t_x, t_y, t_z, \gamma_z]
\end{align}
where $t=[t_x, t_y, t_z]$ represent the translation, $\gamma_z$ the rotation around the \emph{z}-axis. Note that we simplify the model ignoring rotations around \emph{x}(roll) and \emph{y}-axis (pitch)  since the \emph{z}-axes of the operator and the turret are parallel being both on the same plane (i.e. the floor).\\ Finding $\rho$ will allow us to collocate the turret's frame pose (position and orientation) into the human's frame.\\
First, we define the human frame $\{H\}$ the same way we did in section \ref{sec:1.2}. The reference frame {B} is an arbitrary fixed reference frame in which the turret reports the position of the laser projected dot (in section \ref{sec:1.1} this is the \textbf{base\_frame}). With that given, we ask the human to point at the moving laser dot and keep following it with a pointing gesture for a short period of time $\tau$; we collect the pointing rays $r_i^{\{H\}}$ in the reference frame $\{H\}$ and corresponding laser positions $P_i^\{B\}$ in the frame $\{R\}$, after which a coordinate transformation $T*$ between the two frames is estimated using an optimization procedure
