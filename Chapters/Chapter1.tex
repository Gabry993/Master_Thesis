...
\begin{itemize}
    \item \textbf{i} as the distance from the \textbf{pan\_frame} origin to the projection of the laser point on the \textbf{base\_frame};
...
\begin{align}
	x=& i\cos{(pan)}\label{eq:x}\\
	y=& i\sin{(pan)} \label{eq:y}
\end{align}
...
\begin{align}
    H-z =& d\by\cos(tilt)\\
	i =& d\by\sin(tilt) \label{eq:dsin}\\
	i=& \sqrt{x^2+y^2}
\end{align}
Thus, we have that:
\begin{align}
    tilt =& \arctan\bigg(\frac{i}{H-z}\bigg) \label{eq:tiltik}\\
\end{align}
