...
In addition to the \ac{relloc} system, we implemented a basic \acs{PID} controller to move the kobuki. The library to interface with the robot is available online \cite{kobuki:ROS}. That simple controller, contained in the \textbf{kobuki\_go\_to\_goal} module, is very important as, while it computes the velocity command to move the kobuki, it also computes the new goal for the turret, in order to keep pointing the destination while the robot is moving, leveraging on its own odometry. Figure \ref{fig:goToGoalDemo} tries to explain that.\\ 
